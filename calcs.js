...
function getChanges(arr, initial) {
  return arr.map((v, i) => {
...
function getPortfolioValues(dividends, orders, user, initialPortfolio) {
...
function getCurrentPrice(orders, def = 0) {
...
function dividendsPerRound(dividends, orders, user, initialPortfolio, portfolio) {
  return dividends.map((d,i) => {
...
function getTotalPortfolio(dividends, orders, user, initialPortfolio) {
...
function getPortfolio(orders, user, initialPortfolio) {
  if (orders.length === 0) {
...
function getPortfolioChange(pair, user) {
...
// TODO: filter by game???
function getOpenOrders(orders, round) {
...
function getMatch(order, orders) {
...
function getMatches(order, orders) {
...
function currentPosition(round, dividends, orders, user, initialPortfolio) {
...
function canMakeOffer(order, position) {
  if (order.type === 'bid') {
